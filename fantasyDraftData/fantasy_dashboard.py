...
if compare_players:
    ...
    st.dataframe(comp_df[["Ovr", "POS", "Team", "Total_Points"] + weekly_columns], use_container_width=True)

# --- TRADE VALUE ANALYZER ---
st.sidebar.markdown("---")
trade_player_1 = st.sidebar.selectbox("Trade Analyzer: Player 1", ["None"] + sorted(df["Name"].unique()))
trade_player_2 = st.sidebar.selectbox("Trade Analyzer: Player 2", ["None"] + sorted(df["Name"].unique()))
if trade_player_1 != "None" and trade_player_2 != "None" and trade_player_1 != trade_player_2:
    st.markdown('<div class="section-title">Trade Value Analyzer</div>', unsafe_allow_html=True)
    p1 = df[df["Name"]==trade_player_1].iloc[0]
    p2 = df[df["Name"]==trade_player_2].iloc[0]
    st.write(f"**{trade_player_1}**: {p1['Total_Points']} pts, Ovr {p1['Ovr']}, POS {p1['POS']}")
    st.write(f"**{trade_player_2}**: {p2['Total_Points']} pts, Ovr {p2['Ovr']}, POS {p2['POS']}")
    if abs(p1['Total_Points']-p2['Total_Points']) < 20:
        st.success("This trade is likely fair.")
    elif p1['Total_Points'] > p2['Total_Points']:
        st.warning(f"{trade_player_1} is more valuable. Consider asking for more in return.")
    else:
        st.warning(f"{trade_player_2} is more valuable. Consider asking for more in return.")


# --- MOBILE-FRIENDLY LAYOUT ---
mobile_mode = st.sidebar.checkbox("Mobile-Friendly Layout (Compact Mode)")
if mobile_mode:
    st.markdown('<style>body, .main, .block-container { max-width: 100vw !important; padding: 0.5rem !important; } .kpi-card { margin-bottom: 0.5rem !important; } </style>', unsafe_allow_html=True)
...
ss_df = df[df["POS"]==start_sit_pos].sort_values(by=start_sit_week, ascending=False).head(5)
st.markdown(f'<div class="section-title">Start/Sit Recommendations ({start_sit_pos}, {start_sit_week})</div>', unsafe_allow_html=True)
...
if not sleeper_df.empty:
    st.dataframe(sleeper_df[["Ovr", "Name", "POS", "Team", "Total_Points"]], use_container_width=True)
